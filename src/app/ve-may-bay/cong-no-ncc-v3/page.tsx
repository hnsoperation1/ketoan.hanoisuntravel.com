...
// bảng phụ "Hành khách trong tin nhắn" + bảng lô công nợ đầy đủ) ở chỗ:
// KHÔNG còn bảng phụ đó nữa. Bấm 1 tin nhắn ở panel trái → bảng lô công nợ
// (RawBatchesView, dùng lại y hệt v1/v2) tự LỌC chỉ còn các dòng cùng tin
// nhắn đó (onlyShowRelated, xem raw-shared.tsx), và nút "Chọn" để gán mã
// khách/giá của đúng pax tương ứng giờ nằm NGAY trên từng dòng đã lọc
...
export default function CongNoVeV3Page() {
  ...
  // Gán mã khách TAY cho 1 dòng trong lô raw (qua nút "Chọn" ngay trên dòng
  // đã lọc). matchedBookingId khác null chỉ khi chọn từ 1 candidate cụ thể
  // — giữ vết truy vết. giaMua/giaBan lấy đúng từ pax trong tin nhắn (kế
  // toán sửa lại sau qua ô Giá mua/Giá bán sẵn có trên dòng đó nếu cần).
  ...
  useEffect(() => {
    setBreadcrumb(<span className="text-sm font-semibold text-gray-700">Đầu vào công nợ NCC v3</span>)
    ...
    <div className="absolute inset-0 flex flex-col px-5">
          ...
          {NCC_TABS.map(n => (
            <button key={n} type="button"
              onClick={() => {
                setNccFilter(n)
                resetWizard()
                // Đổi tab NCC = đổi hẳn danh sách lô đang xem — panel/lọc
                // đang thuộc về 1 dòng của lô NCC CŨ, không còn liên quan gì
                // tới tab mới, phải đóng lại chứ không thể giữ nguyên.
      ...
      <div className="flex-1 min-h-0 flex items-stretch pt-2">
        ...
        {/* Tay cầm kéo giãn NẰM NGAY TRONG khoảng trống giữa panel và bảng
            (xem giải thích chi tiết ở page.tsx của v1). */}
        <div
